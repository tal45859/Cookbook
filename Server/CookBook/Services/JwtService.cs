...
namespace CookBook.Services
{
    public class JwtService
    {
        ...
        private TimeSpan TokenNumMinutesToExtend = new TimeSpan(1, 1, 0);//הזמן שאנחנו באוויר
        ...
        public string GenerateToken(string tokenPrimaryValue, string pRole)
        {
            ...
            else
            {
                tokenDescriptor = new SecurityTokenDescriptor
                {
                   ...
                   {
                    new Claim(TokenPrimaryKey, tokenPrimaryValue),
                    new Claim(ClaimTypes.Role,"Classic")
                }),
